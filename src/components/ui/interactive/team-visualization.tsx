...
export function TeamVisualization() {
  ...
  const teams = [
    {
      id: "engineering",
      name: isVerySmall ? "Ing" : "Ingénierie",
      icon: Code,
      color: "text-orange-500", // Changed to orange-500
    ...
    },
    {
      ...
      name: "Design",
      icon: Palette,
      color: "text-yellow-500", // Changed to yellow-500
      count: isVerySmall ? 4 : isMobile ? 6 : 12,
    },
    {
      id: "product",
      name: isVerySmall ? "Prod" : "Produit",
      icon: Briefcase,
    ...
    },
    {
      ...
      icon: MessageSquare,
      color: "text-red-500", // Changed to red-500
  ...
  useEffect(() => {
    ...
    const members = Array.from({ length: team.count }, (_, i) => ({
      id: i,
      name: `Membre d'équipe ${i + 1}`,
      role:
        team.id === "engineering"
          ? ["Frontend", "Backend", "DevOps", "QA"][i % 4]
          : team.id === "design"
            ? ["UI", "UX", "Visuel", "Motion"][i % 4]
            : team.id === "product"
              ? ["Manager", "Propriétaire", "Analyste", "Stratège"][i % 4]
              : ["N1", "N2", "N3", "Manager"][i % 4],
    ...
    <div className="bg-gray-900 rounded-lg border border-gray-800 p-3 md:p-6 h-full">
      <div className="flex justify-between items-center mb-3 md:mb-6">
        <div className="flex items-center gap-1 md:gap-2">
          <Users className="h-4 w-4 md:h-5 md:w-5 text-orange-500" /> {/* Changed to orange-500 */}
          <h3 className="font-medium text-xs sm:text-sm md:text-base">Équipe IOI</h3> {/* Changed to IOI */}
        </div>
        <div className="text-[8px] sm:text-xs text-gray-400">{isVerySmall ? "60+" : "60+ Membres d'équipe"}</div>
      </div>
      ...
      <div className="bg-gray-800/30 rounded-lg p-2 sm:p-3 md:p-4 mb-3 md:mb-6">
        <div className="flex justify-between items-center mb-1.5 sm:mb-2 md:mb-3">
          <h4 className="text-[10px] sm:text-xs md:text-sm font-medium">
            Équipe {activeTeam ? teams.find((t) => t.id === activeTeam)?.name : "Toutes"}
          </h4>
          <span className="text-[8px] sm:text-[10px] md:text-xs text-gray-400">
            {activeTeam ? teams.find((t) => t.id === activeTeam)?.count : 0} {isVerySmall ? "" : "membres"}
          </span>
        </div>
              ...
              <div
                className={`relative w-4 h-4 sm:w-6 sm:h-6 md:w-8 md:h-8 rounded-full bg-gray-800 flex items-center justify-center ${
                  activeTeam === "engineering"
                    ...
                    : activeTeam === "design"
                      ? "text-yellow-500" // Changed to yellow-500
                      : activeTeam === "product"
                        ? "text-green-500"
                        : "text-red-500" // Changed to red-500
                }`}
              >
                ...
                <motion.div
                  className={`absolute inset-0 rounded-full border ${
                    activeTeam === "engineering"
                      ? "border-orange-500" // Changed to orange-500
                      : activeTeam === "design"
                        ? "border-yellow-500" // Changed to yellow-500
                        : activeTeam === "product"
                          ? "border-green-500"
                          : "border-red-500" // Changed to red-500
                  }`}
      ...
      <div className="grid grid-cols-2 gap-2 sm:gap-3 md:gap-4">
        <div className="bg-gray-800/50 p-1.5 sm:p-2 md:p-3 rounded-lg">
          <h4 className="text-[8px] sm:text-[10px] md:text-xs font-medium mb-1 md:mb-2">
            {isVerySmall ? "Collab" : "Collaboration d'équipe"}
          </h4>
            ...
            <div className="w-full bg-gray-700 rounded-full h-1 sm:h-1.5 md:h-2.5">
              <motion.div
                className={`h-full rounded-full ${
                  activeTeam === "engineering"
                    ? "bg-orange-500" // Changed to orange-500
                    : activeTeam === "design"
                      ? "bg-yellow-500" // Changed to yellow-500
                      : activeTeam === "product"
                        ? "bg-green-500"
                        : "bg-red-500" // Changed to red-500
                }`}
                initial={{ width: 0 }}
                animate={{
                  width: `${
